...
def tentar_fluxo_revisao_com_ia(telefone, texto):
    ...
    # Proteção para não desviar errado quando o cliente só está preenchendo campos do fluxo
    if etapa_atual == "revisao_modelo" and limpar_texto(texto):
        ...
        # Só permite desvio de intenção se não for um preenchimento simples
...
def webhook():
    ...
    if clientes[telefone].get("atendimento_humano"):
        if texto_normalizado in ["menu", "oi", "ola", "olá", "voltar", "inicio", "início", "bom dia", "boa tarde", "boa noite"]:
            resetar_cliente(telefone)
            enviar_menu(telefone)
